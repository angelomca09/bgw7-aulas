...
func TestSimulatorMock_CanCatch(t *testing.T) {
	t.Run("case 1: get implemented value", func(t *testing.T) {
		// Arrange
		sim := simulator.NewSimulatorMock()
		sim.CanCatchFunc = func(hunter, prey *simulator.Subject) (canCatch bool) {
			return true
		}
		shark := hunter.NewWhiteShark(100, &positioner.Position{X: 0, Y: 0, Z: 0}, sim)
		prey := prey.NewTuna(50, &positioner.Position{X: 100, Y: 100, Z: 100})
	...
	t.Run("case 2: get default value", func(t *testing.T) {
		// Arrange
		sim := simulator.NewSimulatorMock()
